...
- `.github/workflows/update-v8.yml` — The whole update flow in one workflow:
  detects a newer stable V8, computes the next module version, opens a **draft** PR
  early (so the PR exists no matter what — bumping `V8_VERSION` + `ModuleVersion`),
  builds + packages each target into `v8-<target>.tar.zst`, uploads them as Actions
  artifacts, commits `internal/v8dist/manifest.json` (pinning the run id + sha256),
  then — in the same run — `validate` installs those exact artifacts into `.v8/`
  (the `go generate` flow) and runs `make test`/`make coverage` on a native runner
  per arch, and finally marks the PR ready. A build or validate failure leaves the
  PR as a draft rather than deleting it.
